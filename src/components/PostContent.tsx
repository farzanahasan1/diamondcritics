...
function injectPinterestButtons(html: string, slug: string): string {
  const pageUrl = `https://diamondcritics.com/${slug}`;
  const parts: string[] = [];
  let remaining = html;
  while (remaining.length > 0) {
    const imgIdx = remaining.search(/<img/i);
    if (imgIdx === -1) { parts.push(remaining); break; }
    const before = remaining.slice(0, imgIdx);
    // Skip if this img is inside an <a> tag (linked image — would create nested <a>)
    const lastAOpen = before.lastIndexOf('<a ');
    const lastAClose = before.lastIndexOf('</a>');
    const insideLink = lastAOpen > lastAClose;
    const imgEnd = remaining.indexOf('>', imgIdx) + 1;
    const attrs = remaining.slice(imgIdx + 4, imgEnd - 1);
    const srcMatch = /src="([^"]+)"/.exec(attrs);
    if (insideLink || !srcMatch || /class="/.test(attrs)) {
      parts.push(remaining.slice(0, imgEnd));
      remaining = remaining.slice(imgEnd);
      continue;
    }
    ...
    const description = titleAttr ? `${titleAttr} — ${alt}` : alt;
    const pinUrl = `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(pageUrl)}&media=${encodeURIComponent(mediaUrl)}&description=${encodeURIComponent(description)}`;
    parts.push(before);
    parts.push(`<span class="pin-wrap"><img${attrs}><a href="${pinUrl}" target="_blank" rel="noopener noreferrer" class="pin-btn" aria-label="Save to Pinterest"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C5.373 0 0 5.373 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738a.36.36 0 0 1 .083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.632-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg> Pin</a></span>`);
    remaining = remaining.slice(imgEnd);
  }
  return parts.join('');
}
...
export default function PostContent({ type, data, related }: Props) {
  ...
  const { toc, html: tocHtml } = buildToc(data.contentHtml);
  const contentHtml = type === "post"
    ? injectPinterestButtons(tocHtml, data.slug)
    : tocHtml;
